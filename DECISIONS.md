...
**Refinement (2026-09-20, Zhang Yimou autopsy):** rule (3) was too tight for film leads, which often spend their opening sentences on plot ("Red Sorghum is a 1988 Chinese film about a young woman…") and for CJK leads whose "lit." abbreviations break the sentence count. The gate now also accepts a *creatorship phrase* anywhere in the lead — "by / starring / debut of <creator>" — while a bare mention still fails (Method Man's "Chris Rock named his special after this song" stays rejected; regression-checked). The poster class (V3-74) also recognizes "historical drama directed by" leads that never say "film." Result: Zhang Yimou 18→22/22 with his own *Red Sorghum* restored.

**Refinement 2 (2026-09-22, Going All the Way autopsy — Tony's catch):** the creatorship phrase opened an adaptation hole — a film's article names the source author ("screenplay by Dan Wakefield, based on his novel"), so a 1997 poster landed on the 1970 novel card. Two rules: (5) **the article decides what the thing is; the card decides whether that is acceptable** — the earliest kind noun after "is a/an/the" (album / television / film / novel) classifies the article, and a literature card refuses a film or TV article, a film card refuses a novel, a music card refuses either. The class label now follows the article's kind, never the card's medium (which is how the poster had been filed as a "jacket"). (6) When the card knows its medium, the kind-qualified title is looked up first — "Going All the Way (novel)" before bare "Going All the Way". Corpus repair: 30 adaptation stamps stripped, 6 posters relabeled; the re-run under the new rules re-found 56 correct assets across the 27 affected subjects (CORRECTIONS.md). The novel itself has no image on Wikipedia, so Vonnegut's card is now honestly empty.
